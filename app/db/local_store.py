...
Mirrors just the PK/SK/GSI1PK/GSI1SK item shape used by the real table —
this is not a general DynamoDB emulator. It only backs put_item/get_item/
query_gsi1, which is everything case submission and the vet directory
need; the AI worker's conditional update_item() calls (idempotent
claiming, writing triage results) aren't covered here, since the worker
needs SQS + Gemini regardless — both unavailable in the same "no AWS"
scenario, so there's nothing this fallback could unblock there anyway.
